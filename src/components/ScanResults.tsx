...
const ScanResults = ({ results, onBack, onStartNew }: ScanResultsProps) => {
	...
	const downloadReport = (format: "pdf" | "json") => {
		if (format === "pdf") {
			...
			link.href = "/1.pdf";
			link.download = `revolve-report-${results.certificateId.toLowerCase()}.pdf`;
			document.body.appendChild(link);
			link.click();
			document.body.removeChild(link);
		} else {
			// For JSON format, create and download JSON data
			const jsonData = {
			...
			const link = document.createElement("a");
			link.href = url;
			link.download = `revolve-report-${results.certificateId.toLowerCase()}.json`;
			document.body.appendChild(link);
			link.click();
			document.body.removeChild(link);
						...
						<Card className="shadow-lg border-0 text-center hover-lift">
							...
							<CardContent className="space-y-6">
								...
								<div>
									<Badge
										variant="outline"
										className={`text-xl px-6 py-3 ${getGradeColor(
											results.grade,
										)} border-current`}
									>
										{results.grade}
					...
					<div className="lg:col-span-2 space-y-6">
						<Card className="shadow-lg border-0 hover-lift">
							<CardHeader className="pb-6">
								...
								<p className="text-muted-foreground">
									Detailed breakdown of device components and their
									condition
								</p>
							</CardHeader>
							<CardContent>
								<div className="space-y-4">
									{results.components.map((component, index) => (
										<div
											key={index}
											className="flex items-center justify-between p-5 bg-gradient-secondary rounded-xl border border-border/50 hover-lift"
										>
											...
											<div className="flex items-center space-x-6">
												<div className="text-right min-w-0">
													...
													<Progress
														...
														className="w-28 h-3 mt-2 shadow-sm"
													/>
												</div>
											</div>
										</div>
						...
						<Card className="shadow-lg border-0 hover-lift">
							<CardHeader className="pb-6">
								...
								<p className="text-muted-foreground">
									Get detailed reports in multiple formats for your
									records
								</p>
							</CardHeader>
							<CardContent>
								...
								<div className="mt-6 p-6 bg-accent/5 rounded-xl border border-accent/20">
									<p className="text-sm text-muted-foreground leading-relaxed">
										<strong>Note:</strong> All reports include the
										tamper-proof certificate and compliance
										verification. Keep these documents for audit
										trails and resale purposes.
									</p>
								</div>
							</CardContent>
						</Card>
